...
TEST_F(ExrcTest, copyUptoNull) {
    // The constructor of string from a const char* should copy characters
...
TEST_F(ExrcTest, undefBehaviorWhenCharNotNullTerminated) {
    std::cerr << "⚠️  WARNING: This test case demonstrates a common undefined behavior issue where a non-null-terminated char array is passed to construct string.\nThe behavior is unpredictable and may cause a crash." << std::endl;
...
TEST_F(ExrcTest, copyNcharsFromPosOfCStrMethod) {
    string s4(cp + 6, 5);  // Copy 5 characters starting at `cp[6];`
...
TEST_F(ExrcTest, copyNpXcharsFromPosOfCStrMethod) {
    ...
    string s5(cp + 6, 25);  // Copy 5 characters starting at `cp[6];`
...
TEST_F(ExrcTest, copyNpXcharsFromStr) {
    string s1(cp);
    string s6(s1, 6, 5);  // Copy 5 characters starting at `s1[6];`
...
TEST_F(ExrcTest, copyNpXcharsFromStr2) {
    string s1(cp);
    string s6(s1, 6);  // Copy characters starting at `s1[6];` to the end
...
TEST_F(ExrcTest, copyNpXcharsFromStr3) {
    string s1(cp);
    string s6(s1, 6, 20);  // ok, copies only to end of s1; s7 == "World!!!"
...
TEST_F(ExrcTest, copyNpXcharsFromStrThrowOutOfRange) {
    ...
    ASSERT_THROW(string s6(s1, 16), std::out_of_range);
}

TEST_F(ExrcTest, substrFullCopy) {
    string s2 = s.substr();

    ASSERT_EQ(s2, "Hello World!!!");
    ASSERT_EQ(s2.size(), 14);
}

TEST_F(ExrcTest, substrStartPosCountCopy) {
...
TEST_F(ExrcTest, substrStartPosCopy) {
...
TEST_F(ExrcTest, substrStartPosCountCopy2) {
    string s3 = s.substr(6, 11);

    ASSERT_EQ(s3, "World!!!");
    ASSERT_EQ(s3.size(), 8);
}

TEST_F(ExrcTest, substrCopyOutOfRange) {
    ASSERT_THROW(string s3 = s.substr(16), std::out_of_range);
}